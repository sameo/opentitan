...
enum {
  ...
  kRomPatchRegionMatchTableSize = 32 * 2 * sizeof(uint32_t),  // 32 * 2 DWORDs
...
rom_patch_remap(const rom_patch_t *patch) {
  for (uint32_t i = 0; i < kRomPatchNumRemapPerPatch; i++) {
    ...
    OT_DISCARD(rom_printf(
        "Configured Ibex remapping 0x%x->0x%x (%d bytes) %x\n", m_base, r_base,
        patch_size_bytes, rom_patch_region_locked(match)));
  }
