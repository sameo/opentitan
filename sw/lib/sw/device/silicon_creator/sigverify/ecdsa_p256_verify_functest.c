...
rom_error_t ecdsa_p256_verify_ok_test(void) {
  ...
  rom_error_t result = sigverify_ecdsa_p256_verify(&kEcdsaSignature, &kEcdsaKey,
                                                   &digest, &flash_exec);
  LOG_INFO("flash_exec 0x%x vs 0x%x result 0x%x", flash_exec, kSigverifyEcdsaSuccess, result);
...
rom_error_t ecdsa_p256_verify_negative_test(void) {
  ...
  LOG_INFO("flash_exec 0x%x vs 0x%x", flash_exec, kSigverifyEcdsaSuccess);
